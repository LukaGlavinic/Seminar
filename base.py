...
from torchvision.datasets import CIFAR10, DatasetFolder
...
class Base(object):
    ...
    def train(self, schedule=None):
        ...
        work_dir = osp.join(self.current_schedule['save_dir'], self.current_schedule['experiment_name'] + '_' + time.strftime("%Y_%m_%d_%H_%M_%S", time.localtime()))
        ...
        for i in range(self.current_schedule['epochs']):
            print(f"Epoch {i}/{self.current_schedule['epochs']}")
            for batch_id, batch in enumerate(train_loader):
                print(f"Batch {batch_id}/{self.current_schedule['batch_size']}")
